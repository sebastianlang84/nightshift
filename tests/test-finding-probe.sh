...
finding() { # fingerprint code_sig item
  jq -nc --arg r "$REPO" --arg fp "$1" --arg cs "$2" --arg i "$3" \
...
[ "$(rows)" = 4 ] || { echo "todos must list all four open findings, listed $(rows)" >&2; exit 1; }
...
[ "$(rows)" = 3 ] || { echo "todos must drop the closed finding" >&2; exit 1; }

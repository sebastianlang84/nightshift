...
# The gate runs the repo's declared `test_cmd` in the WORKTREE, right before the commit:
#   fails  -> no branch anywhere, ledger `tests-failed`, digest shows it, the finding stays unlatched
...
# --- 1. a failing suite blocks the ship ---------------------------------------

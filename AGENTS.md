...
- **Orchestrator:** [`bin/nightshift.sh`](bin/nightshift.sh) — the night loop: harvest → verify (close open findings) → recon → explore → fix↔review → finalize (repo's own `test_cmd` gates the ship, ADR 0022, then push `nightshift/*`).

...
parse silently becomes a 0. Naive local timestamps use the fallback too: Python and GNU date choose
different sides of an ambiguous DST fold (and disagree on nonexistent wall-clock times). Negative
fractional epochs also fall back because Python's `int()` truncates toward zero while `date +%s`
floors them. These remain rare paths: nightshift writes modern `date -Iseconds` timestamps with an
explicit UTC offset.
"""
...
def parse_epoch(ts: str) -> int:
    ...
    try:
        ...
        # Preserve GNU date's local-time decisions around DST folds/gaps. Nightshift's own
        # `date -Iseconds` rows are timezone-aware, so this compatibility path does not dilute the
        # normal batch speedup.
        if parsed.tzinfo is None:
            return _date_fallback(s)
        epoch = parsed.timestamp()
        # `int(-0.5)` is 0 but `date +%s` returns -1. The callers currently ignore non-positive
        # service epochs, yet tsv-epochs promises field-for-field compatibility for every row.
        if epoch < 0:
            return _date_fallback(s)
        return int(epoch)

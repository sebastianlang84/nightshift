...
- **Dedup (§1.7):** a finding already in the ledger (by the Runner-canonical
  `sorted(files):type:symbol` fingerprint — line numbers excluded, ADR 0014) is skipped on the next
  night — the anti-nag mechanism.
